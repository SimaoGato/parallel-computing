...
void domain_initialize(void) {
  ...
  for (int_t i = 0; i < local_M; i++) {
    for (int_t j = 0; j < local_N; j++) {
      ...
      real_t delta =
          sqrt(((global_i - M / 2.0) * (global_i - M / 2.0)) / (real_t)M +
...
void border_exchange(void) {
  ...
  // Create column datatype to send and receive the border columns (helpful for
  // east-west communication)
  MPI_Type_vector(local_M, 1, local_N + 2, MPI_DOUBLE, &column_type);
  MPI_Type_commit(&column_type);

  // Send to north, receive from south
  MPI_Sendrecv(&U(0, 0), local_N, MPI_DOUBLE, north, 0, &U(local_M, 0), local_N,
               MPI_DOUBLE, south, 0, cart_comm, MPI_STATUS_IGNORE);

  // Send to south, receive from north
  MPI_Sendrecv(&U(local_M - 1, 0), local_N, MPI_DOUBLE, south, 0, &U(-1, 0),
               local_N, MPI_DOUBLE, north, 0, cart_comm, MPI_STATUS_IGNORE);

  // Send to west, receive from east
  MPI_Sendrecv(&U(0, 0), 1, column_type, west, 0, &U(0, local_N), 1,
               column_type, east, 0, cart_comm, MPI_STATUS_IGNORE);

  // Send to east, receive from west
  MPI_Sendrecv(&U(0, local_N - 1), 1, column_type, east, 0, &U(0, -1), 1,
               column_type, west, 0, cart_comm, MPI_STATUS_IGNORE);
...
void domain_save(int_t step) {
  ...
  // Create an MPI datatype to represent the local block of data governed by
  // this process
  ...
  MPI_Offset offset =
      (cart_coords[0] * local_M * N + cart_coords[1] * local_N) *
      sizeof(real_t);
  MPI_File_set_view(fh, offset, MPI_DOUBLE, local_block, "native",
                    MPI_INFO_NULL);
  ...
  MPI_File_write_all(fh, local_data, local_M * local_N, MPI_DOUBLE,
                     MPI_STATUS_IGNORE);
