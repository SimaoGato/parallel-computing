...
__global__ void device_calculate(int *device_pixel, double xleft, double step,
                                 double yupper) {
...
int main(int argc, char **argv) {
  ...
  cudaMemcpy(d_pixel, device_pixel, XSIZE * YSIZE * sizeof(int),
  ...
  dim3 dimGrid((XSIZE + BLOCKX - 1) / BLOCKX, (YSIZE + BLOCKY - 1) / BLOCKY);

  device_calculate<<<dimGrid, dimBlock>>>(d_pixel, xleft, step, yupper);
  ...
  cudaMemcpy(device_pixel, d_pixel, XSIZE * YSIZE * sizeof(int),
